...
public ResponseEntity<BigDecimal> sacar(
    @PathVariable Long id,
    @RequestBody BigDecimal valor
) {
    BigDecimal novoSaldo = usuarioService.sacar(id, valor);
    return ResponseEntity.ok(novoSaldo);
}

@GetMapping("/{id}/extrato")
public ResponseEntity<List<Transacao_Entity>> extrato(
    @PathVariable Long id
) {
    List<Transacao_Entity> historico = usuarioService.extrato(id);
